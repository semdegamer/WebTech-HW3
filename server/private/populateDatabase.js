...
const generateCoursesData = () => {
  return [
    ...
    { name: "Machine learning", description: "An advanced course about machine learning", code: "INFOB3ML", instructor: "Thijs van Ommen" },
  ];
};
    ...
    try {
      ...
      const programInsertPromises = programs.map((program) => {
        return new Promise((resolve, reject) => {
          ...
          [program.name, program.description], function(err) {
            if (err) {
              reject('Error inserting program:', err);
            }
            resolve({ programId: this.lastID, name: program.name }); // Capture the inserted program ID
          });
        });
      });
      ...
      const courseInsertPromises = courses.map((course) => {
        return new Promise((resolve, reject) => {
          ...
          [course.name, course.description, course.code, course.instructor], function(err) {
            if (err) {
              reject('Error inserting course:', err);
            }
            resolve({ courseId: this.lastID, code: course.code }); // Capture the inserted course ID
          });
        });
      });
      ...
      const studentInsertPromises = students.map((student) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          [randomProgram.programId, student.firstName, student.lastName, student.email, student.passwordHash, student.birthDate, student.photoLink, student.hobbies], function(err) {
            if (err) {
              reject('Error inserting student:', err);
            }
            resolve({ studentId: this.lastID, programId: randomProgram.programId });
          });
        });
      });
